...
parsed = 'path to parsed files'
completed = 'path to final output'
...
def excel_convert():    
    ...
    for excel in excel_files:
        ...
        df = pd.read_excel(excel, engine = 'openpyxl') # if only the first sheet is needed.

        #Specify which columns to select by column header
        filtered_columns = ['Column1', 'Column2', 'etc']
        df2 = df.reindex(columns = filtered_columns)
        df2.to_csv(out) 
